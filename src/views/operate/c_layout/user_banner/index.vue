    ...
    >
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          ...
          >
            <i
              @click="userChangePosition(-1, scope.$index, scope.row.id)"
              class="el-icon-top"
          ...
          <el-button
            type="danger"
            size="mini"
            @click="delBtn(scope.row.id, scope.$index)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
import {
  ...
  sortBanner,
  delBanner,
} from "@/api/operate/c_layout/banner";
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    userChangePosition(type, index, id) {
      let list = this.list;
      if (type == -1) {
      ...
      sortBanner({ type: this.type, sort: type, id });
    },
    addMeal() {
      this.$router.push("/operate/c_layout/user_banner_add");
    },
    goEdit(id) {
      this.$router.push(`/operate/c_layout/user_banner_edit?id=${id}`);
    },
    getList() {
      this.loading = true;
